...
from scripts.relace.model_configs import smallmoe_1B_A390M
...
def build_config(opts, overrides: List[str]) -> ExperimentConfig:
    ...
    # )
    model_config = smallmoe_1B_A390M(
    ...
    data_loader_config = NumpyDataLoaderConfig(
        global_batch_size=1024 * 1024,  # NOTE: this is specified in tokens, not instances
        #global_batch_size=1024 * 1024,  # NOTE: this is specified in tokens, not instances
        seed=0,
        num_workers=4,
    )
    ...
    train_module_config = TransformerTrainModuleConfig(
        rank_microbatch_size= 8 * 1024,  # NOTE: this is specified in tokens, not instances
